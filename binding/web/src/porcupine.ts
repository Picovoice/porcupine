...
export class Porcupine {
   ...
   * @returns A new PorcupineKeyword object containing the trained model. Pass this to `.create()`.
   * Object must have 'forceWrite: false' or be empty for Porcupine to initialize.
   */
  public static async trainWakeWordFromPhrase(
    accessKey: string,
    writePath: string,
    language: string,
    phrase: string,
  ): Promise<PorcupineKeyword> {
    ...
    const res = await fetch(`${PV_API_URL}${language}/api/ppn`, {
      method: 'POST',
      headers: {
    ...
    const file = await open(writePath, 'w');
