# Porcupine Wake Word Engine Demo for STM32F469 (Multiple languages)
...
1. Download `STM32Cube middleware for audio PDM to PCM conversion` and copy it to the project folder. A more detailed guide can be found in the [readme](./stm32f469i-disco/Middlewares/ST/STM32_Audio/Addons/PDM/readme.txt) file.
...
3. Copy the contents of the array inside the `.h` header file and update the `DEFAULT_KEYWORD_ARRAY` value in [/stm32f469i-disco/Inc/pv_params.h](./stm32f469i-disco/Inc/pv_params.h) in the language section for which the model is trained.
