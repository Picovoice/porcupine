...
void BSP_AUDIO_IN_TransferComplete_CallBack(uint32_t Instance) {
    if (Instance == 1U) {
        SCB_InvalidateDCache_by_Addr((uint32_t *) &record_pdm_buffer[AUDIO_IN_PDM_BUFFER_SIZE / 2], AUDIO_IN_PDM_BUFFER_SIZE * 2);
...
void BSP_AUDIO_IN_HalfTransfer_CallBack(uint32_t Instance) {
    if (Instance == 1U) {
        SCB_InvalidateDCache_by_Addr((uint32_t *) &record_pdm_buffer[0], AUDIO_IN_PDM_BUFFER_SIZE * 2);
