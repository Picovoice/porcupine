      ...
      (tester) async {
        for (int t = 0; t < testData['tests']['singleKeyword'].length; t++) {
          ...
          try {
            ...
            expect(ex, equals(null), reason: "Failed to initialize Porcupine for ${language}: ${ex}");
            return;
          }

          String audioPath = "assets/test_resources/audio_samples/${keyword.replaceAll(' ', '_')}.wav";
          ...
          for (int i = 0; i < (pcm.length - frameLength); i += frameLength) {
            int detection = await porcupine.process(pcm.sublist(i, i + frameLength));
            if (detection != -1) {
    ...
    testWidgets('Test multipleKeyword all languages',
      (tester) async {
        for (int t = 0; t < testData['tests']['multipleKeyword'].length; t++) {
          ...
          porcupine.delete();
          expect(detections.length, equals(groundTruth.length), reason: "Number of detections for ${language} was incorrect");
          expect(detections, equals(groundTruth), reason: "Porcupine detections don't match truth for ${language}");
        }
      });
  });
}
