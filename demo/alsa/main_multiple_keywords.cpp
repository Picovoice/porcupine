...
const char * const KEYWORD_PATHS[] = {"../../resources/keyword_files/pineapple_linux.ppn", "../../resources/keyword_files/avocado_linux.ppn"};
float sensitivities[] = {0.50f, 0.50f};
...
int start_porcupine()
{
	...
	int num_items = sizeof(KEYWORD_PATHS) / sizeof(KEYWORD_PATHS[0]);
    pv_status_t status = pv_porcupine_multiple_keywords_init(MODEL_PATH, num_items, KEYWORD_PATHS, sensitivities, &porcupineObject);
...
int listen()
{
	...
	while (!stop)
	{
		...
		if (err < 0) {
            ...
            pv_porcupine_multiple_keywords_process(porcupineObject,wav_data, &keyword_index);

			if (keyword_index != -1) {
				// Detected keyword. Do something!
				cout << "Detected keyword! (" << keyword_index << ")" << endl;
			}
		}
	}
	return 0;
