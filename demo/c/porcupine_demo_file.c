...
void print_usage(const char *program_name) {
    fprintf(
        ...
        "SENSITIVITY -a ACCESS_KEY -w WAV_PATH\n",
        program_name
    );
}
...
int picovoice_main(int argc, char *argv[]) {
    ...
    int c;
    while ((c = getopt_long(argc, argv, "l:m:k:t:a:w:", long_options, NULL)) !=
           -1) {
        switch (c) {
        case 'l':
            library_path = optarg;
    ...
    if (f.sampleRate != (uint32_t)pv_sample_rate_func()) {
        fprintf(
            stderr, "audio sample rate should be %d\n.", pv_sample_rate_func()
        );
        exit(1);
    }
    ...
    while ((int32_t)drwav_read_pcm_frames_s16(
               &f, pv_porcupine_frame_length_func(), pcm
           ) == pv_porcupine_frame_length_func()) {
        ...
        total_processed_time_usec +=
            (pv_porcupine_frame_length_func() * 1e6) / pv_sample_rate_func();

        if (keyword_index != -1) {
            fprintf(
                stdout,
                "detected at %.1f seconds\n",
                (double)frame_index * pv_porcupine_frame_length_func() /
                    pv_sample_rate_func()
            );
        }
        frame_index++;
