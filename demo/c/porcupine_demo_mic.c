...
static void print_usage(const char *program_name) {
    fprintf(
        stderr,
        "Usage : %s -l LIBRARY_PATH -m MODEL_PATH -k KEYWORD_PATH -t "
        "SENSITIVITY -a ACCESS_KEY -d AUDIO_DEVICE_INDEX\n"
...
void show_audio_devices(void) {
    ...
    pv_recorder_status_t status =
        pv_recorder_get_audio_devices(&count, &devices);
    if (status != PV_RECORDER_STATUS_SUCCESS) {
        fprintf(
            stderr,
            "Failed to get audio devices with: %s.\n",
...
int picovoice_main(int argc, char *argv[]) {
    ...
    int c;
    while ((c = getopt_long(argc, argv, "sl:m:k:t:a:d:", long_options, NULL)) !=
           -1) {
        switch (c) {
        case 's':
            show_audio_devices();
    ...
    const char *(*pv_status_to_string_func)(pv_status_t) =
        load_symbol(porcupine_library, "pv_status_to_string");
    ...
    int32_t (*pv_sample_rate_func)() =
        load_symbol(porcupine_library, "pv_sample_rate");
    ...
    int32_t (*pv_porcupine_frame_length_func)() =
        load_symbol(porcupine_library, "pv_porcupine_frame_length");
    ...
    const char *(*pv_porcupine_version_func)() =
        load_symbol(porcupine_library, "pv_porcupine_version");
    ...
    pv_status_t porcupine_status = pv_porcupine_init_func(
        access_key, model_path, 1, &keyword_path, &sensitivity, &porcupine
    );
    ...
    pv_recorder_status_t recorder_status = pv_recorder_init(
        device_index, frame_length, 100, true, true, &recorder
    );
...
#define NULL_TERMINATED (-1)
    ...
    for (int i = 0; i < argc; ++i) {
        ...
        int arg_chars_num = WideCharToMultiByte(
            CP_UTF8,
